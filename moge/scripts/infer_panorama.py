...
@click.option('--merge_method', '--merge-method', type=click.Choice(['raycast', 'poisson']), default='raycast', help='Method for merging panorama 2D maps: "raycast" (default, fast, metric-accurate) or "poisson" (legacy gradient integration).')
@click.option('--planar_floor/--no_planar_floor', '--planar-floor/--no-planar-floor', 'planar_floor', default=True, help='Fit a planar ground floor and snap floor points to prevent ground splatter. Defaults to True.')
@click.option('--mesh_type', '--mesh-type', type=click.Choice(['equirectangular', 'multiview']), default='equirectangular', help='3D mesh method: "equirectangular" (default, continuous airtight 360 room mesh with texture) or "multiview" (multi-view perspective meshes).')
@click.option('--align_scales/--no_align_scales', '--align-scales/--no-align-scales', 'align_scales', default=True, help='Normalize scale across all 12 views using the ground plane to eliminate inter-view floating steps. Defaults to True.')
...
@click.option('--use_cache', '--use-cache', is_flag=True, help='Use cached splitted views from <output>/splitted to skip neural inference and continue directly with 2D merging and 3D mesh building.')
