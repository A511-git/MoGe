...
| **`--merge_method`** | `raycast` \| `poisson` | `raycast` | Method for fusing the 2D panorama maps.<br>• **`raycast` (default)**: Fast, closed-form spherical cosine raycasting that preserves exact metric scale and avoids polar distortion or drift.<br>• **`poisson`**: Legacy unanchored gradient integration. |
| **`--mesh_type`** | `equirectangular` \| `multiview` | `equirectangular` | Method for 3D mesh reconstruction.<br>• **`equirectangular` (default)**: Builds a single, continuous, airtight 360-degree room mesh with seamless $360^\circ$ horizontal wrapping, no floating cutout steps, and high-resolution photographic texture mapping.<br>• **`multiview`**: Directly rotates the 12 perspective meshes into world space. |
| **`--align_scales`** | Flag | `True` | Normalizes monocular depth scale across all 12 views using the ground plane height consensus.<br>• **`--align_scales` (default)**: Eliminates inter-view scale drift and stepped wall/ceiling misalignments.<br>• **`--no_align_scales`**: Disables inter-view scale normalization. |
| **`--planar_floor`** | Flag | `True` | Automatically fits a horizontal ground floor plane using surface normals and snaps floor points.<br>• **`--planar_floor` (default)**: Completely eliminates ground splatter and grazing-angle tearing, producing a clean, flat floor.<br>• **`--no_planar_floor`**: Leaves ground points unconstrained. |
